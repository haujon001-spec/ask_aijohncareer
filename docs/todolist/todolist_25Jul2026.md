...
2. **History — ability to delete run files.** **Scoped, not implemented.** User confirmed: company-level delete removes `data_processed/<Company>/` outputs only — source JD text (`data_raw/jd/txt/`) and blueprint cache (`src/data/jd/`) are left alone since other runs of the same role may still need them. Full architecture + open questions (header-button restructure needed, no trash/undo pattern exists, no stable run ID, concurrent-run guard needed): `docs/guides/JDPORTALHISTORYDELETE_SCOPING_25JUL2026.md`.
...
4. History — ability to delete run files (`[Company]` level) — **scoped**, ready for an implementation session. See `docs/guides/JDPORTALHISTORYDELETE_SCOPING_25JUL2026.md`.
5. Authoritative `john_profile.json` update capability — **scoped**, larger epic, ready for an implementation session. See `docs/guides/JOHNPROFILEUPDATE_SCOPING_25JUL2026.md` (includes the `/api/consolidate` security fix, folded in per user decision).
