...
function ChatPage() {
  ...
  useEffect(() => {
    const checkBackend = async () => {
      try {
        ...
        console.log('🔍 [ChatPage] Checking backend at:', url);
        const resp = await fetch(url, { method: 'GET' });
        console.log('✅ [ChatPage] Backend health check response:', resp.status);
        if (!resp.ok) throw new Error('Backend not reachable');
        setBackendError(false);
      } catch (e) {
        console.error('❌ [ChatPage] Backend health check failed:', e);
  ...
  const handleSendMessage = async (userMessage) => {
    ...
    console.log('📝 [ChatPage] Selected model:', selectedModel);
    ...
    try {
      ...
      console.log('🚀 [ChatPage] Sending request to backend:');
      ...
      console.log('📥 [ChatPage] Response received:');
      console.log('   Status:', response.status, response.statusText);
      console.log('   Content-Type:', response.headers.get('content-type'));

      if (!response.ok) {
        const errorText = await response.text();
        console.error('❌ [ChatPage] HTTP error response:', errorText);
        throw new Error(`HTTP ${response.status}: ${errorText.substring(0, 200)}`);
      }

      const data = await response.json();
      console.log('✅ [ChatPage] Parsed response:', {
        model: data.model,
      ...
      if (!data.answer || data.error || hasNoAnswer) {
        console.warn('⚠️ [ChatPage] No valid answer from LLM. Error:', data.error, 'HasNoAnswer:', hasNoAnswer);
      ...
      console.log('✅ [ChatPage] Message added to chat');
    } catch (error) {
